<template>
  <view>
    <cu-custom bgColor="bg-blue" :isCustom="true">
      <view slot="content">{{ UploadNum == 0 ? "修改房源" : "正在上传..." }}</view>
    </cu-custom>
    ...
    <view class="cu-form-group">
      <view class="grid col-4 grid-square flex-sub">
        <view v-for="(item, index) in videoList" :key="9" class="bg-video" :data-index="index" @tap="ViewVideo">
          <video :src="item.url" object-fit="cover" :controls="false" :muted="true" :autoplay="true" :loop="true"
            class="radius"></video>
          <view class="cu-tag bg-red" @tap.stop="DelVideo" :data-index="0">
    ...
    <view :class="'cu-modal ' + (isShowModal == true ? 'show' : '')">
      <view class="cu-dialog">
        ...
        <view class="cu-bar bg-white">
          <button class="cu-btn line-grey margin-left" @tap="closePage">返回</button>
          <button class="cu-btn line-blue text-blue" @tap="hideModal">
            使用当前模板
          </button>
...
<script>
  ...
  export default {
    ...
    async onLoad(options) {
      let houseId = options.houseId || "2907a0c34e9243598614ef828eecd20d"
    ...
    methods: {
      async gethouse(houseId) {
        ...
        await this.http
          ...
          .then((res) => {
            ...
            let house = res.data;
            this.house = house;
            this.imgList = house.imgUrl;
            this.videoList = house.videoUrl;
            this.houseTypeIndex = [
              house.houseType.rooms,
              house.houseType.halls,
              house.houseType.toilets,
            ];
            this.payMethodIndex = [house.payMethod.mortgage, house.payMethod.pay];
            house.tags.forEach((item) => {
              this.tempTags += item + " ";
            });
            house.subways.forEach((item) => {
              this.tempSubways += item + " ";
            });
            house.districts.forEach((item) => {
              this.tempDistricts += item + " ";
            });
            // 还差地区
            /* #ifdef MP */
            this.house.region = {
              province: house.region.province,
              city: house.region.city,
              area: house.region.area,
            };
            /* #endif */
            ...
            for (let i = 0; i < regionIndex[0].length; i++) {
              if (regionArr[0][i] == house.region.province) {
                regionIndex[0] = i; //找到province对应index
                regionArr[1] = Object.getOwnPropertyNames(
                  pca[house.region.province]
                ); //找到该province所有city
                break;
              }
            }
            for (let i = 0; i < regionIndex[1].length; i++) {
              if (regionArr[1][i] == house.region.city) {
                regionIndex[1] = i; //找到province对应index
                regionArr[2] = pca[currentProv][currentArea]; //找到该city所有area
                break;
              }
            }
            for (let i = 0; i < regionIndex[2].length; i++) {
              if (regionArr[2][i] == house.region.area) {
                regionIndex[2] = i; //找到area对应index
                break;
              }
            }
            /* #endif */
      ...
      async chooseImage() {
        uni.chooseImage({
          ...
          success: (res) => {
            console.log("选择图片");
            console.log(res);
            this.uploadImg(res.tempFilePaths, res
              .tempFiles); //  this.uploadFileTest(res.tempFilePaths)
      ...
      async uploadVideo(tempFilePath, tempfile) {
        ...
        let fileName = Date.now() + "-" + uuidlast + "." + pathArr[pathArr.length - 1];
        ...
        if (this.UploadNum > 0) {
          ...
          if (house.title == "") {
            ...
            for (let key in house) {
              if (house[key] == null || house[key] == "None") {
                // 不提交值为null的字段，不做修改
                delete house[key];
              }
            }
